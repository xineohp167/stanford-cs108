...
public class Cracker {
	...
	public static void main(String[] args) {
		if (args.length != 3 && args.length != 1) {
			...
			System.err
					.println("Use java cracker.Cracker [hash] [passLength] [numOfThreads]");

			System.err.println("Laaunching cracker with defaults args.");

			Cracker cracker = new Cracker();
			cracker.launcher(DEFAULT_HASH_TO_CRACK, DEFAULT_PASS_LENGTH,
					DEFAULT_NUM_THREADS);
		}
	...
	private void setPassLength(String passLength) {
		try {
			...
			System.err
					.println("Wrong input format for pass length. Use integers.");
			System.exit(-1);
		}
	}
	...
	private void setNumOfThreads(String input) {
		try {
			...
			System.err
					.println("Wrong input format for number of threads. Use integers.");
			System.exit(-1);
		}
	}
	...
	private void startWorkers() {
		...
		for (int i = 0; i < numOfThreads; i++) {
			char[] segment;
			if (i != (numOfThreads - 1)) {
				segment = Arrays.copyOfRange(CHARS, startPoint,
